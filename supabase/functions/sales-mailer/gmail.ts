 ...
 * Supports dynamic sender email for bulk sales mailing
 */
...
interface EmailParams {
  ...
  subject: string;
  htmlContent: string;
  senderEmail?: string; // Optional: allows dynamic sender
}
...
export async function sendGmailNotification(params: EmailParams): Promise<EmailResult> {
  try {
    ...
    const defaultSenderEmail = Deno.env.get("GMAIL_SENDER_EMAIL") || "ankit@hushh.ai";
    
    // Use provided sender email or fall back to default
    const senderEmail = params.senderEmail || defaultSenderEmail;
    ...
    console.log(`Email sent successfully to ${params.to}, message ID:`, result.id);
